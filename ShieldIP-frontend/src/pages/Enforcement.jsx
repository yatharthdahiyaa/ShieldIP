...
export default function Enforcement() {
  ...
  const buildNotice = useCallback((action, violation) => {
    const conf = Math.round((violation.match_confidence > 1 ? violation.match_confidence : (violation.match_confidence || 0.9) * 100));
    const platform = violation.platform || 'the platform';
    const url = violation.url || '[URL on file]';
    const vid = violation.violation_id || 'N/A';
    const today = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
    if (action.id === 'Claim Monetize') {
      return `CONTENT MONETIZATION CLAIM\nDate: ${today}\nClaim Reference: CLM-${vid.slice(0,8).toUpperCase()}\n\nTo: ${platform} Content Monetization Team\n\nShieldIP, acting as authorized agent for the registered rights holder, hereby submits a monetization claim for the content located at:\n${url}\n\nThis content (Violation ID: ${vid}) was identified with ${conf}% fingerprint match confidence against registered intellectual property. Under the platform's Content ID / rights management system, we request that all ad revenue generated by this content be redirected to the verified rights holder effective immediately.\n\nBank routing and rights holder details are on file. Please confirm claim processing within 5 business days.\n\nShieldIP Enforcement Team`;
    }
    if (action.id === 'Flag Legal') {
      return `LEGAL ESCALATION MEMORANDUM\nDate: ${today}\nCase Reference: LEG-${vid.slice(0,8).toUpperCase()}\nPriority: HIGH\n\nFlagged for Legal Review\n\nViolation Summary:\n- Platform: ${platform}\n- URL: ${url}\n- Violation ID: ${vid}\n- Match Confidence: ${conf}%\n- Region: ${violation.region || 'Global'}\n\nBasis for Escalation:\nThe detected infringement level (${conf}% confidence) and platform reach indicate wilful, commercial-scale piracy that exceeds the threshold for automated takedown. This case is being escalated for evaluation of statutory damages under 17 U.S.C. § 504(c), which provides for up to $150,000 per work infringed in cases of willful infringement.\n\nRecommended Next Steps:\n1. Retain litigation counsel\n2. Preserve digital evidence (screenshots, view counts, revenue estimates)\n3. Issue formal cease-and-desist letter\n4. File with appropriate jurisdiction\n\nShieldIP Legal Operations`;
    }
    return `DMCA TAKEDOWN NOTICE\nDate: ${today}\nNotice Reference: DMC-${vid.slice(0,8).toUpperCase()}\n\nTo: ${platform} Trust & Safety / DMCA Agent\nRe: Unauthorized Use of Copyrighted Material\n\nDear ${platform} DMCA Agent,\n\nPursuant to 17 U.S.C. § 512(c) of the Digital Millennium Copyright Act, ShieldIP hereby provides formal notification of claimed infringement on behalf of the registered rights holder.\n\nInfringing Content:\n  URL: ${url}\n  Violation ID: ${vid}\n  Detection Confidence: ${conf}% (pHash fingerprint match)\n  Region: ${violation.region || 'Global'}\n\nThe content identified above is an unauthorized reproduction of copyrighted material owned by our client. The infringing material has been detected and verified via automated fingerprint analysis. Our client has not authorized this use.\n\nWe request that you expeditiously remove or disable access to the infringing material. We are providing this notice in good faith and under penalty of perjury that the information is accurate.\n\nFailure to comply within 48 hours may result in further legal action, including injunctive relief and statutory damages.\n\nShieldIP Enforcement Team\nhttps://shieldip.ai`;
  }, []);
  ...
  const handleAction = useCallback(async (action, violation) => {
    ...
    try {
      ...
      if (!key || !action.usesAI || !rl.allowed) {
        setNoticeText(buildNotice(action, violation) + (!rl.allowed ? `\n\n[Rate limit active: ${rl.reason}]` : ''));
        return;
      }
      recordRequest();
      const genAI = new GoogleGenerativeAI(key);
      const model = genAI.getGenerativeModel({ model: 'gemini-2.5-flash' });
      const prompt = `Draft a formal ${action.label} notice for this IP violation. Be professional, legally precise, and do not use placeholders — use the data provided.\n\nViolation data: ${JSON.stringify(violation)}`;
      const result = await model.generateContent(prompt);
      setNoticeText(result.response.text());
    } catch (err) {
      console.error('[Gemini] Enforcement generation failed:', err);
      setNoticeText(buildNotice(action, violation));
